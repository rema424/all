...
- 人物として登場するのは「客」と「店員」の 2 人
- この場面における客と店員は主語（Subject）であって目的語（Object）ではない
- この局所的な部分において客が達成したいことは商品の予約を確定させること
- 局所的に見た場合には達成したいことは購入ではない
- 一回の電話のやりとり（セッション）で複数の商品を予約したい
  - 電話開始から商品の受取までがセッション
- 予約したい各商品に対してそれぞれ個数をリクエストする
- 店員は商品と個数を聞いたら、確保可能な分だけ一度商品の確保を完了させる
  - 例えば商品 A を 10 個リクエストされて在庫が 5 個の場合でも、ひとまずこの 5 個は売り場からバックヤードに持ってきて電話口の客のために確保する・・・(A)
  - この段階では電話口以外の客は商品 A は品切れで購入できない
  - 客が口にした商品は概念としての商品（値オブジェクト）であり、店員が確保する商品はシリアルナンバーの付与された実体（エンティティ）である
- 商品の確保が完了したらリクエストとの差分を客に伝える
  - 商品 A は 20 個要求されて 20 個確保できました
  - 商品 B は 10 個要求されて 5 個確保できました
  - 商品 C は 10 個要求されて 3 個確保できました
- 差分を表示された客はこれを聞いた上で注文内容の「数量」を修正し、注文内容を確定する・・・(B)
  - 商品 C は 3 個中 3 個
  - 商品 B は 5 個中 3 個
  - 商品 A は 20 個中 6 個
  - このとき別の商品の追加はできない
- 数量の変更を聞いた店員は、数量が減った分の在庫を陳列棚に戻す
- 注文を確定したら客は店舗に出向いて商品を受け取る・・・（C）
- 処理（A）から処理（B）への猶予時間は 5 分とし、途中で電話での応答がなくなるなどした場合は取り置いておいた品を陳列棚に戻して電話を切る
- 処理（B）から処理（C）への猶予時間は 30 分とし、客が店頭に現れない場合は取り置いていた品を陳列棚に戻す
  - 時間が過ぎてから客が現れた場合は、時間が経過した旨を客に伝える必要がある
  - 「予約されていません」ではだめ
- 商品の状態
  - 陳列棚または倉庫にある・・・on sale
  - 一回目の在庫確保で数量調整を待っている状態・・・pending（猶予時間 5 分）
  - 数量調整が完了して予約完了状態・・・reserved（猶予時間 30 分）
  - 販売完了状態・・・sold out
